...
一定要让 CoolReadWrite 这个 PermissionProfile  启动的时候能知道自己当下可调用的 CTool工具都有哪些

所有基础工具都必须符合 CToolScope 视野规则。
同时 基础工具不应具备 
不应具备联网、下载、编译可执行文件、运行可执行文件的能力。
### KP
...
- 用于快速定位代码或配置中的相关位置。
- 支持限制搜索深度、最大结果数、是否包含点号开头的普通隐藏文件名等参数。
- `include_hidden` 不能覆盖 CToolScope 的 `hidden` 规则。
...
- 必须经过 CToolScope 的 readwrite 权限检查。
- create、delete、move 权限都从 readwrite 推导，不在 scope.toml 中新增独立权限维度。
- 不能操作 `.cool`、`.cool-system` 等硬保护配置区。
- 不能删除或移动 `CoolWorkspace` 根目录。
- 不能删除或移动 CharacterRoot 关键目录。
- 不能创建可执行文件或二进制文件。
- 删除目录只能删除空目录，不能递归删除。
- 移动目录不能覆盖已有目录。
...
- 不能递归删除。
- 不能删除硬保护路径。
- 不能删除 `CoolWorkspace` 根目录或 CharacterRoot 关键目录。
...
- 不允许覆盖已有目录。
- 不能移动硬保护路径。
- 不能移动 `CoolWorkspace` 根目录或 CharacterRoot 关键目录。
...
- `max_depth`：最大搜索深度。
- `max_results`：最大返回命中数。
- `include_hidden`：是否包含点号开头的普通隐藏文件名；不能覆盖 CToolScope 的 `hidden` 规则。
