...
CoolDir\command.toml         Character 级，低一级
```
...
Character 配置可以添加自己的 Green / Yellow / Red 规则，但不能覆盖系统级更高风险规则。
...
如果命令字符串里包含 `&&`、`||`、`;`、`|`、重定向、子 shell 等控制符，风险分类应按拆分后的每个命令段计算，整组取最高风险。否则可能出现一个字符串里混入高风险命令但被低估的情况。
...
```text
CoolDir\cache\command_request\2026-06-09\
  request_log.md
...
日志目录位于 CoolDir cache 下，建议作为 CToolScope 的 readonly 例外：Codex 可以读，但不能改。
...
Output:
CoolDir\cache\command_request\2026-06-09\00000_yellow_cargo_check.md
